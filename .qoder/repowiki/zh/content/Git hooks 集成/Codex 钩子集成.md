...
## 更新摘要
**变更内容**
- 会话启动钩子从列出活动问题改为提供简洁能力提示
- 移除了复杂的活动问题列表逻辑，简化为静态上下文注入
- 保留了更新检查功能作为可选附加内容
- 减少认知负荷并提高性能表现
...
## 简介
本文件系统性阐述 gxpm 的 Codex 钩子集成方案，重点对比 Codex 钩子与标准 Git 钩子（githooks）的差异，详解会话启动与用户提示提交两个事件钩子的工作原理、配置项与扩展方式，并提供安装步骤、最佳实践与性能优化建议。Codex 钩子通过在用户会话中注入简洁的能力提示，减少认知负荷并提升 AI 辅助开发的效率与一致性。
...
- [templates/codex-hooks/session-start.sh:1-90](file://templates/codex-hooks/session-start.sh#L1-L90)
...
- [test/install-codex-hooks.test.ts:1-320](file://test/install-codex-hooks.test.ts#L1-L320)
...
- **钩子脚本**：会话启动钩子在会话开始时注入简洁的能力提示；用户提示提交钩子在用户输入包含 issue 引用时注入该 issue 的状态与下一步建议
...
- [templates/codex-hooks/session-start.sh:10-90](file://templates/codex-hooks/session-start.sh#L10-L90)
...
- **Codex 钩子安装位置**：在用户级或仓库级的 .codex 目录，而非 .git 钩子目录
- **触发机制**：Codex 需要在配置中开启 feature flag（codex_hooks = true），否则钩子不会触发
- **配置管理**：Codex 钩子通过 hooks.json 描述事件与命令，支持多条目合并，避免覆盖用户原有钩子
...
### 会话启动钩子（SessionStart）- 已更新
**更新** 从列出活动问题改为提供简洁能力提示，减少认知负荷并提高性能

- **输入**：Codex 通过 stdin 传递 session_id、transcript_path、cwd、hook_event_name、model、source 等
- **行为**：检查 cwd 下是否存在 .gxpm/issues，如果存在则生成简洁的能力提示，包含版本信息、schema版本和基本使用指导；否则静默退出
- **输出**：返回结构化 JSON，包含 hookEventName 与 additionalContext 字段

**简化后的上下文内容**：
- 仓库使用的 gxpm 版本和 schema 版本信息
- 基本的使用指导（如 `gxpm issue list` 和 `gxpm issue status <id>`）
- 可选的更新检查通知
...
CheckGXPM --> |是| GetSchema["获取 schema 版本"]
GetSchema --> GetVersion["获取版本信息"]
GetVersion --> BuildContext["构建简洁能力提示"]
BuildContext --> CheckUpdate["检查更新可用性"]
CheckUpdate --> HasUpdate{"有更新？"}
HasUpdate --> |是| AddUpdate["添加更新提示"]
HasUpdate --> |否| SkipUpdate["跳过更新提示"]
AddUpdate --> Output["输出结构化 JSON"]
SkipUpdate --> Output
Exit --> End(["结束"])
Output --> End
```
...
**图表来源**
- [templates/codex-hooks/session-start.sh:13-90](file://templates/codex-hooks/session-start.sh#L13-L90)
...
- **简洁上下文注入**：会话启动钩子只提供必要的能力提示，减少上下文大小
- **hooks.json 合并策略**避免重复注册，减少 Codex 启动时的扫描开销
- **feature flag 检查与写入**仅在需要时进行，且幂等，降低重复安装成本
- **更新检查可选**：通过环境变量控制，不影响基础性能
...
- **性能问题**
  - 检查是否启用了更新检查功能
  - 使用 `GXPM_SESSION_START_DISABLE=1` 环境变量禁用会话启动钩子
...
## 结论
Codex 钩子通过在会话启动与用户提示提交时注入简洁的能力提示，显著减少了认知负荷并提升了 AI 辅助开发的效率。其安装器具备幂等性与合并策略，确保与用户既有钩子共存。结合标准 githooks，可形成"本地 Git 流程 + AI 会话上下文"的双重保障体系。
...
- **使用环境变量控制行为**：
  - `GXPM_SESSION_START_DISABLE=1`：禁用会话启动钩子
  - `GXPM_UPDATE_CHECK_BIN=/path/to/update/check`：指定更新检查程序

**章节来源**
- [scripts/install-codex-hooks.ts:136-164](file://scripts/install-codex-hooks.ts#L136-L164)
- [templates/codex-hooks/session-start.sh:30-90](file://templates/codex-hooks/session-start.sh#L30-L90)
